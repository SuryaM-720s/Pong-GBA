...
    bn::sprite_ptr ball = bn::sprite_items::ball_new.create_sprite(0, 0);
...
        if (bn::keypad::left_held() && center_paddle.x() > -140)
        {
            center_paddle.set_x(center_paddle.x() - 1);
        }
...
        else if (bn::keypad::right_held() && center_paddle.x() < 140)
        {
            center_paddle.set_x(center_paddle.x() + 1);
        }
...
            // if (bn::abs(ball.y() - center_paddle.y()) < 32)
            // {
            //     // We made it!
            //     // ...
...
        if (ball.x() > (center_paddle.x() - 30) && ball.x() < (center_paddle.x() + 30))  // Horizontal collision zone
        {
            // Check if ball is at the paddle's vertical position
            if (bn::abs(ball.y() == 50))  // Vertical collision zone
            {   
                BN_LOG(ball.y());
                BN_LOG(center_paddle.y());
                // Increase score
                score++;
                text_sprites.clear();
                bn::string<32> txt_score = "Score: " + bn::to_string<32>(score);
...
                // Simple bounce - invert the vertical direction
                delta_y = -delta_y;
                
                
                // Play sound effect
...
// and (bn::abs(ball.x() - center_paddle.x()) < 2)